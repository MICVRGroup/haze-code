...
'''读入原始数据并转为list'''
path = 'C:/Users/pro14/Desktop/suoyouwenjian/wenjianjia/chengshi/duoarima/chun/quanniancsv/'

data = pd.read_csv(path+'yuncheng.csv')
...
b1=test_y.T
b=b1.reshape([447])
zhongresults.append([a,b])
data = np.vstack(zhongresults)
#data1=data[:,:,0]
#zhongresults.append([[y_test[:j * BATCH_SIZE], results[:j * BATCH_SIZE]]])
#data = np.vstack(zhongresults)

df = pd.DataFrame(data=data.T, columns=['inv_yHat', 'inv_y'])
df.to_csv('C:/Users/pro14/Desktop/suoyouwenjian/wenjianjia/chengshi/danlstm/5/yc.csv')
...
def scale_inv(raw_data,scale=True):
    #读入原始数据并转为list
    path = 'C:/Users/pro14/Desktop/chengshi/duolstm/1/quanniancsv/'

    data = pd.read_csv(path + 'changzhi.csv')
